...
function formatDistanceToNow(date) {
    ...
    if (isNaN(parsedDate.getTime()) || !regex.test(date)) { // Check for invalid date string
        return "Invalid date";
    }
    ...
    if (diffInMilliseconds < 1000) { // Less than a minute
        return "less than a minute ago";
    } else if (diffInMilliseconds < 60000) { // Less than a minute but more than a second
    ...
    } else if (diffInMilliseconds < 2592000000) { // Less than a month
        const days = Math.round(diffInMilliseconds / 86400000);
        return `${days} day${days > 1 ? 's' : ''} ago`;
    } else if (diffInMilliseconds < 31536000000) { // Less than a year
      ...
      <div class="my-6">
        <div class="flex flex-col gap-2 py-4 pt-0">
            ...
            <a
                ...
                href="workspace/hubs/{{ item.hub.id }}"
                preload="mouseover"
            >
                <div class="flex w-full flex-col gap-1">
                <div class="flex items-center">
                    <div class="flex items-center gap-2">
                    <div class="font-semibold">{{ item.hub.name }}</div>
                    </div>
                    <div class="updated-at ml-auto text-xs">
                    {{ item.updatedAt }}
                    </div>
                </div>
                <div class="text-xs font-medium">{{ item.hub.description }}</div>
                </div>
